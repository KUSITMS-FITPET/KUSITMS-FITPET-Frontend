...
export const fetchAdminList = async (page: number = 1) => {
  try {
    ...
    const response = await axios.get(
      `/api/v1/fitpetAdmin/master?page=${page}`,
      {
        headers: {
...
export const createAdmin = async (adminData: NewAdmin, roleMaster: boolean) => {
  if (!roleMaster)
    throw new Error('권한이 없습니다. 관리자 추가는 마스터 권한이 필요합니다.')
...
export const deleteAdmin = async (adminId: string, roleMaster: boolean) => {
  if (!roleMaster)
    throw new Error('권한이 없습니다. 관리자 삭제는 마스터 권한이 필요합니다.')

  try {
    const token = getToken() // 토큰 가져오기
    const response = await axios.delete(
      `/api/v1/fitpetAdmin/master/${adminId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`, // Authorization 헤더에 토큰 추가
        },
